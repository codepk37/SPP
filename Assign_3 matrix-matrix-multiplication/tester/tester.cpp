...
int main(int argc, char *argv[]){
	...
	try{
		...
		constexpr const float error_threshold = 20;
		...
		for(std::uint32_t i=file_size-remaining; i < file_size; i++){
			...
			if(diff > error_threshold) __terminate_gracefully("Solution has higher error than required: " + std::to_string(diff)+"  "+std::to_string(error_threshold) );
		}
